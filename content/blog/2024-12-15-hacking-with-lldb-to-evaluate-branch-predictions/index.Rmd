...
and it often depends on the assumptions you make when comparing them.
...
Working this through sequentially: (TODO: remove?)
...
I'm sure there are standard benchmarks and I think
there are much better tools to gather this type of data as well.
But here's my method using some of the basic tools on my 2020 Intel MacBook Air.

### Disassembling executables

Which executables? system executables are off limits. SIP, crusutils, etc.

I started out with wasting a bunch of time trying to disassemble
with LLDB. It turns out you can only disassemble unit a function or frame,
but I wanted the entire program. 

Luckily, otool does disassemble the entire program.

### Instruction set 

What instructions are branches? How do we know if they will be taken?

### Tracing with LLDB

Scripting with python and lldb commands. 
Should I be using more python scripting? https://lldb.llvm.org/use/python-reference.html

Order of operations is incredibly important.

Stepping through instructions seems like a good idea (avoid 
having to know more details about the instruction set, have
to gather less data), but the lldb scripting breaks
