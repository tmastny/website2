...
At first I thought that the normal counter version had to do this as well,
...
and writes can be batched together into a store buffer before being written to the cache.
...
meaning you are likely to have some false sharing! 
I think it's funny that the mutex is itself susceptible to false sharing.
...
But as we saw, adjacent mutexes are susceptible to false sharing.
The solution is to pad the mutexes so they are in different cache lines.

I compared the performance of the regular mutex version and the padded version,
and the padded version only performed better when working across the P and E cores 
(in fact I got a consistent, but small penalty when using padding on just the P cores). 
...
* there is a lot of cache line contention: in the previous examples,
  we have up to 8 threads bouncing the same cache line. In this case,
  only two lines can be false shared.
