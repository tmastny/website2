...
the second version is 0.68% percent faster. 

This effect is called false sharing, 
and can occur multiple CPUs are working with variables that are close to each other in memory.
...
While one thread holds the lock, if other threads
try to acquire the lock, they must wait until it is unlocked.
...
to finish modifying the variable.
...
announce it's intent to the memory controller. Other CPUs with the same variable
receive that message, invalidate their local copy, and acknowledge the write.
When the writiing CPU receives the acknowledgements, it writes to it's copy
of the variable. 

![](cache-coherence.svg)

If another CPU2 or CPU3 wants to write or read to that variable, 
it will see that it's local copy is invalid and the data must
be synchronized from CPU1's cache before the operation can complete. 
This synchronization is much faster than a trip to main memory, 
but is still an measurable amount of time. 
...
And that's the cause of false sharing. 
...
false sharing slows down the program by about 0.68%. 
...
We can see this effect when we measure false sharing that crosses P and E cores.
...
I think this is a fasincating topic and highlights so interesting
low-level details of cache design on modern CPUs. 