...
So the overall space complexity is `O(n)`^[Thanks for Robert Ehrlich for the correction].


## Real-world Implementation

Carry-lookahead adders can also be constructed as intermediate blocks
and chained together. For example, [Harris and Harris](https://www.sciencedirect.com/topics/computer-science/carry-propagate-adder)
describe eight 4-bit carry-lookahead blocks chained together to compute
a 32-bit sum.

This design is a nice compromise, saving gates by avoiding one
full-width 32-bit carry-lookahead network^[Which can still be practical!],
while still achieving some speedup from the 4-bit carry-lookahead in each
block.
