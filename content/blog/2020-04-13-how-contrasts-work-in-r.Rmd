...
```{r echo = FALSE}
boxer <- function(x, col, outcome) {
  x %>%
    ggplot(aes(fct_rev({{ col }}), {{ outcome }}, group = {{ col }})) +
    geom_boxplot() +
    xlab(NULL) +
    geom_hline(yintercept = mean(pull(x, {{ outcome }}), na.rm = TRUE), color = "blue") +
    coord_flip()
}
```
...
For that, we need `emmeans::emmeans`:
...
Are some performing better than others, or is there one bad apple driving
most of this delay?
...
or they company might want to measure the impact of a new release or strategy.
...
Consecutive comparisons highlight the years there were significant peaks
...
This one's also the most unique, because the unit of comparison is not an
individual member of a category, but rather groups compared to other groups.

This type of analysis is useful not non-linear trends, where you want
