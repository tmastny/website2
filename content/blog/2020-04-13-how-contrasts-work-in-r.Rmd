...
Business example one: we care about air time and we want to understand
the differences in air time between carriers. 

Let's get a sense of the differences between airlines with some exploratory 
analysis.
...
Boxplots can be really useful for visualizing continuous variables between groups.
...
We'll fit a linear model using the airlines as predictors.
...
The difference between Delta and JetBlue is not significant. 
American has about 23 minutes less air time than United, but it's not significant.
...
If we were using average air time across airlines as a benchmark 
...
This time we'll look at average number of airline seats per year.
It is very common to track metrics year-by-year, 
but this isn't a great example. However, we can use it to
illistrate the technique.
...
rather than a long-term over 10 years. For example, 
if the business made a major change in one year, 
they want to be able to determine if it made a statistically significant 
difference in later years.

Another example is usability and experience: how do usability ratings by user
change after the user has been on the platform 1 year, compared to 2 or 3?
A big difference in usability in year 3 compared to year 1 and 2 might
indicate users take a long time to understand the platform. 
...
In this case the model is very confident in before and after differences
because the number of seats is consistently increasing year-over-year.
