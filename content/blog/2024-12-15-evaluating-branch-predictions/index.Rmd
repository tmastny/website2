...
is to also save the *history* of whether the last n branches were taken or not.
The main advantage is that any correlated patterns in the history
can be used to predict the next branch.^[Check out Dan Luu's [post](https://danluu.com/branch-prediction/)
for a great overview of branch prediction algorithms.]
...
The detail of this approach that has fascinated me is how to
combine the address and history effectively to form a key
...
In this post, I'll share how I explored the differences between the two methods,
starting with algorithmic analysis more generally, examining concrete examples,
and even proving mathematically that gshare might be **worse** than concatenation!
...
Let's start with the extreme case: what's the maximum
...
of the address. That would mean each key would be `0000`
in the 4-bit case.
If this happens *all* keys would collide.

On the other hand, with a 4-bit concatenation scheme,
...
But maximum number of collisions isn't the full story.
...
of collisions, not just the worst case.
