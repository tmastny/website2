...
If you aren't familar with B+ trees, check out [this resource](https://cs186berkeley.net/notes/note4/)

Let's take search: say we want to find the key 55. 
...
Sequential access and comparsion is fast: 
the next key is likely to be in the cache and
there's only a single integer comparsion per key
...
> For example, one thing that might limit sequential scan 
> speeds is fetching the next cache line.  
> After scanning through 64 bytes, the CPU must fetch the 
> next line, which might be in main memory.
> So you might think you need to add 100ns per 64 bytes processed. 
...
> Likewise, if you are scanning across many pages, you might cause
> another type of cache miss in the Translation Lookaside Buffer (TLB), 
> which could cause another 3-4 main memory loads.
> In practice, modern CPUs have a cache prefetcher that can predict
> the next line and fetch it before it is needed. CPUs are very good at
> sequential data access, so I'll stick with 1ns per key for now.

Following a pointer is expensive. 
The pointer's data is unlikely to be in the cache,
so the the data will have to be fetched from main memory. 
I'll estimate the time to load a pointer's data to be 100ns. 

Even though following pointers are expensive, 
the total number of pointers we have to follow is 
always "small".
1 trillion keys have less than 30 levels, regardless of the order. 
Finding any key in that situation would only take about
30 * 100ns = 3 microseconds. 
...
it would take 16 minutes to scan 1 trillion keys.

For fun let's estimate the optimal order for a B+ tree with 100,000,000 keys,
and then compare it to a benchmark I ran on on my M3 Macbook Air. 
...
> data structure: they balance efficienct search, insertion, and deletion
> and work well with computer architecture. 
...
\text{Time} &= (\text{Height} \times \text{pointer_chase_cost}) + (\text{Height} \times b / 2 \times \text{comparison_cost})
\end{align*}
$$
