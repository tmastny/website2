...
And I'm excited to share what I learned about IPython and Siuba in the progress.
...
and `.` is used for column names or attributes.

See [Why not `hp`?](#why-not-hp) for more.

# Completion

So how does RStudio tab-complete work, and how did we figure out how to do it
in Siuba?
...
We'll need a way to inspect Python objects and find relevant context.
...
First, we want find all (if any) data frames are defined in the environment.

There a couple different ways to do this, but IPython has built
short-cut using 
...
referenced in the most recent input.

![](carbon-code.svg)

I think there's a lot of room to improve here. For example, RStudio definitely
...
data frame you want to complete to make sure it's working.


## Completion

This is my favorite part: remember how I said that `_` is a placeholder
for the data frame?
...
This also has a side benefit of working really well with pandas Series methods,
so things like `_.hp.<TAB>` also provide suggestions.

# Appendix

## Python and Basic Completion
...
Simple customization can be added by defining a method named
[`__dir__`](https://docs.python.org/3/reference/datamodel.html#object.__dir__)
in the object. 

These methods are what [IPython](https://ipython.readthedocs.io/en/stable/config/integrating.html)
and Jupyter notebooks use by default. 
And pandas provides [tab-completion](https://github.com/pandas-dev/pandas/pull/5050/files#diff-9c718a39eb63c1bfac4fbeacf2906ffdR56-R60)
for data frame columns by defining the the custom `__dir__` method

Siuba's needs are more complicated. There's no column information stored
within the data pronoun `_`. We need to somehow search the surrounding context
for a suitable data frame. That requires IPython.
