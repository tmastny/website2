...
At some point, 
all our our customers are submitting and printing uploaded data.
...
logs %>%
  customer_funnel("first-firstafter") %>%
  summarise(
    avg_days_to_submit = mean(days_to_submit), 
    avg_days_to_print = mean(days_to_print),
    avg_time_to_complete_funnel = mean(days_to_submit + days_to_print)
  )
```

Something is definitely wrong. On average, customers are waiting printing 9 days
after uploading. This gap is too big: the data is out-of-date after 5-6 days and
...
We care about more than the first upload.
...
However they are still taking almost 6 days on average to complete the funnel. That's right on the edge of being useless.
...
Around 25% of data uploaded isn't printed off as a report.
What's going on? 
